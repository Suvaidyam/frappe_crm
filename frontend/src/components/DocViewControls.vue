...
function getParams() {
  ...
  const column_field = _view?.column_field || 'type'
...
if (allowedViews.includes('group_by')) {
  defaultViews.push({
    ...
    onClick() {
      viewUpdated.value = false
      router.push({ name: route.name, params: {...route.params, viewType: 'group_by' },query: route?.query })
    },
  })
}
if (allowedViews.includes('kanban')) {
  defaultViews.push({
    ...
    onClick() {
      viewUpdated.value = false
      router.push({ name: route.name, params: {...route.params, viewType: 'kanban' },query: route?.query })
    },
  })
}
